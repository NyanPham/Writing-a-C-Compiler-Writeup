...
public:
        ...
        const std::shared_ptr<Assembly::Instruction> &instr)
    {
        ...
        case NodeType::JmpCC:
            return instr;
        case NodeType::Pop:
            throw std::runtime_error("We shouldn't use this yet");
        default:
            return instr;
        }
    }
    ...
    void addPseudoNodes(
        ...
        AssemblySymbols::AsmSymbolTable &asmSymbol)
    {
        auto nds = getPseudoNodes(aliasedPseudos, instructions, asmSymbol);
    ...
    void addEdges(
        ...
        AssemblySymbols::AsmSymbolTable &asmSymbol)
    {
        // Flatten all instructions in all basic blocks
        ...
        const std::vector<std::shared_ptr<Assembly::Instruction>> &instructions)
    {
        ...
        // Step 1: Count pseudo register usage in instructions
    ...
    void colorGraph(Graph &g)
    {
        ...
        int k = static_cast<int>(R.allHardregs.size());
    ...
    Settings &settings)
{
    ...
    addPseudoNodes(graph, aliasedPseudos, instructions, asmSymbol);
    auto cfgGraph = cfg::instructionsToCFG(fnName, instructions);
    ...
    Settings &settings)
{
    ...
    Graph graph = buildInterferenceGraph(fnName, aliasedPseudos, instructions, asmSymbol, settings);

    // 2. Dump graph (graphviz and ncol)
    auto dbg = R.debugSettings(settings);
    ...
    // 3. Add spill costs
    addSpillCosts(graph, instructions);

    // 4. Color graph
    colorGraph(graph);

    // 5. Build register map and record callee-saved usage
    auto pseudoToRegMap = makeRegisterMap(graph, fnName, asmSymbol);

    // 6. Replace pseudoregs
    auto result = replacePseudoRegs(instructions, pseudoToRegMap);
    return result;
}
    ...
    for (const auto &tl : program->getTopLevels())
    {
        if (tl->getType() == Assembly::NodeType::Function)
        {
            ...
            RegAllocImpl gpAlloc({"gp",
                                 ...
                                 canonicalPseudoMap, canonicalHardregsMap);
            RegAllocImpl xmmAlloc({"xmm",
