...
// Helper to run compiler.exe on a source file with --validate and expect success
inline void typecheck_run_compiler_on(const std::string &srcFile)
{
...
TEST_CASE(Chapter5ValidSemantic, "chapter_5", "--validate")
{
    ...
    {
        print_compile_process("Validate", "valid", srcFile);
        typecheck_run_compiler_on(srcFile);
    }
}
...
TEST_CASE(Chapter11ValidSemantic, "chapter_11", "--validate")
{
    std::vector<std::string> srcFiles = {
...
TEST_CASE(Chapter11ValidSemanticExtraCredit, "chapter_11", "--validate")
{
    std::vector<std::string> srcFiles = {
...
TEST_CASE(Chapter18ValidSemantic, "chapter_18", "--validate")
{
    ...
    for (const auto &srcFile : srcFiles)
    {
        std::cout << "Validate compiling: " << srcFile << '\n';
        print_compile_process("Validate", "valid", srcFile);
        typecheck_run_compiler_on(srcFile);
    }
}
